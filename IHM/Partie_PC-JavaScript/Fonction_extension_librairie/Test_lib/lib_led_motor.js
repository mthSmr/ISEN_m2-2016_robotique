...
(function(ext) {
    ...
    function motor(direction, quelMoteur, pwmMot, temps){
        ...
        if (direction === "forward"){
            ...
            var msg = new Uint8Array([MOTOR, direct, motur, pwmMot, temps])
        device.send(msg.buffer);
    }
    ...
    ext.ledOn = function(numLed, R, G, B){
        ...
        if (R > threshold || G > threshold || B > threshold) return;{
            return ledOn(numLed, R, G, B); }
    }

    ext.motor = function(direction, quelMoteur, pwmMot, temps){
        var threshold = 255;
        if (pwmMot > threshold ||  temps > 120) return; else {
            return motor(direction, quelMoteur, pwmMot, temps); }
    }
